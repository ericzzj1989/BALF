...
def main():
    args, cfg = config_gopro_eval.parse_rel_blur_data_seq_config()
    ...
    imgsName = sorted(os.listdir(args.real_data_path))
    iterate = tqdm(imgsName, total=len(imgsName), desc="Real Data Blur Detection")

    # for path in sort(folder_paths):

    for imgName in iterate:
        image_path = os.path.join(args.real_data_path, imgName)
        ...
        if Path(vis_file).exists():
            print('The detection image already exists!'.format(str(vis_file)))
            continue
        ...
        if args.output_detection_img:
            ...
            # logger.info('Saving detection image in {}'.format(str(vis_file)))
        
        if image_pts.shape[0] >= 900:
            logger.info('Saving detection image in {} with name {}'.format(str(vis_file), str(Path(image_path).name)))
