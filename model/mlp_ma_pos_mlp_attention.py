...
class MABAttentionBlock(nn.Module):
    """Residual channel attention block. Contains LN,Conv,lRelu,Conv,SELayer."""
    def __init__(self, in_ch, reduction_factor=4, lrelu_slope=0.2, grid_gmlp_factor=2,
                 block_gmlp_factor=2, input_proj_factor=2):
        super(ResidualChannelAttentionBlock, self).__init__()
        self.norm = nn.LayerNorm(in_ch)
        self.conv1 = nn.Linear(in_ch, in_ch*input_proj_factor)
        self.act = nn.LeakyReLU(negative_slope=lrelu_slope)
        self.grid_gmlp_layer = GridGmlpLayer(in_ch, grid_size, grid_gmlp_factor, dropout_rate)
        self.block_gmlp_layer = BlockGmlpLayer(in_ch, block_size, block_gmlp_factor, dropout_rate)
        self.conv2 = nn.Linear(in_ch*input_proj_factor, in_ch)
        self.calayer = CALayer(in_ch, reduction_factor=reduction_factor)
...
class MLPMAPOS(nn.Module):
    def __init__(self, model_cfg):
        super(MLPMAPOS, self).__init__()
        ...
        # )
        self.norm = nn.BatchNorm2d(en_embed_dims[4])
        self.act = nn.ReLU(inplace=True)


        self.dense1_1 = nn.Linear(en_embed_dims[4], en_embed_dims[4])
        self.norm1_1 = nn.BatchNorm2d(en_embed_dims[4])
        self.act1 = nn.ReLU(inplace=True)
        self.dense1_2 = nn.Linear(en_embed_dims[4], out_channels)
        self.norm1_2 = nn.BatchNorm2d(out_channels)

        self.dense2_1 = nn.Linear(en_embed_dims[4], en_embed_dims[4])
        self.norm2_1 = nn.BatchNorm2d(en_embed_dims[4])
        self.act2 = nn.ReLU(inplace=True)
        self.dense2_2 = nn.Linear(en_embed_dims[4], 2)
        self.norm2_2 = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        x = self.down2(x)
        x = self.down3(x)
        x = self.act(self.norm(self.down4(x))) # b c h w

        x = x.permute(0, 2, 3, 1) # b h w c

        score = self.dense1_1(x) # b h w c
        score = score.permute(0, 3, 1, 2) # b c h w
        score = self.norm1_1(score) # b c h w
        score = self.act1(score) # b c h w
        score = score.permute(0, 2, 3, 1) # b h w c
        score = self.dense1_2(score) # b h w c
        score = score.permute(0, 3, 1, 2) # b c h w
        res_score_map = self.norm1_2(score)


        pos = self.dense2_1(x) # b h w c
        pos = pos.permute(0, 3, 1, 2) # b c h w
        pos = self.norm2_1(pos) # b c h w
        pos = self.act2(pos) # b c h w
        pos = pos.permute(0, 2, 3, 1) # b h w c
        pos = self.dense2_2(pos) # b h w c
        pos = pos.permute(0, 3, 1, 2) # b c h w
        res_position = self.norm2_2(pos)


        return res_score_map, res_position